pipeline {
    ...
    stages {
        ...
        stage('Docker Run Container') {
            steps {
                sh 'sudo docker run -p 1111:8080 -e GS_SLACK_TOKEN=$GS_SLACK_TOKEN -e GS_SLACK_DIRECT_MESSAGE_ID=$GS_SLACK_DIRECT_MESSAGE_ID --name notification-service --network=host --detach notification-service'
            }
        }    
    }
}
